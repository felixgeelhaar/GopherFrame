...
func TestPropertyFilterCommutativity(t *testing.T) {
	properties := gopter.NewProperties(nil)
	
	properties.Property("Filter operations are commutative", prop.ForAll(
		func(data []testRow, threshold1, threshold2 float64) bool {
			if len(data) == 0 {
				return true
			}
			
			// Ensure threshold1 < threshold2 for a valid range
			if threshold1 > threshold2 {
				threshold1, threshold2 = threshold2, threshold1
			}
			
			// Skip if thresholds are effectively equal (empty range)
			const epsilon = 1e-9
			if math.Abs(threshold1-threshold2) < epsilon {
				return true
			}
